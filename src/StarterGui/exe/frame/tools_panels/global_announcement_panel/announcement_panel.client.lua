...
frame.confirm.MouseButton1Click:Connect(function()
	if frame.scroll.description.container.textbox.Text == "" then
		...
		if not db and not filtering then
			...
			banit_events.global_announce:FireServer(frame.scroll.description.container.textbox.Text,
				properties.announcement_icon.Value, properties.announcement_duration.Value, properties.fullscreen.Value)

			exe_module:tools_panels("global_announcement", false)
		...
		if filtering then
			exe_module:notify("Filtering...", 3, "rbxassetid://11432840962")
		end
	end
end)
...
frame.scroll.description.container.textbox.FocusLost:Connect(function()
	...
	if configs.GLOBAL_ANNOUNCEMENT.filter_string and frame.scroll.description.container.textbox.Text ~= "" then
		filtering = true
		...
		tween_service:Create(frame.scroll.description.filtering, info, {Position = UDim2.fromScale(.5, 2)}):Play()
	end
end)
...
frame.scroll.duration.textbox.FocusLost:Connect(function()
	local limit = configs.GLOBAL_ANNOUNCEMENT.duration_limit
	...
	if num and (num > 0 and num <= limit) then
		properties.announcement_duration.Value = num
		...
		properties.announcement_duration.Value = 5
...
frame.scroll.options.fullscreen.MouseButton1Click:Connect(function()
	if properties.fullscreen.Value == 1 then
		properties.fullscreen.Value = 0
	else
		properties.fullscreen.Value = 1
	end
end)

properties.fullscreen.Changed:Connect(function(value)
	frame.scroll.options.fullscreen.check.Visible = (value == 1)
end)
...
for i, icon in pairs(frame.scroll.icons.sets:GetChildren()) do
	if icon:IsA("ImageButton") then
		...
		icon.MouseButton1Click:Connect(function()
			properties.announcement_icon.Value = icon.icon.Image
		end)
		...
		properties.announcement_icon.Changed:Connect(function(value)

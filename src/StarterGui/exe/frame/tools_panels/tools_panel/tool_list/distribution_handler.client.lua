			...
			button.InputBegan:Connect(function(input)
				if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
					...
					input.Changed:Connect(function()
						if input.UserInputState == Enum.UserInputState.End then
							if properties.dragging.Value and properties.hovering.Value then
								...
								banit_events.give_tool:FireServer(recipient, button.properties.tool.Value)
							end
					...
					if pressing then
						...
						properties.dragging_element.tool_name.value.Text = button.Name
		...
		if button:IsA("ImageButton") then
			...
			button.MouseEnter:Connect(function()
				if properties.dragging.Value then
					...
					tween_service:Create(properties.dragging_element.tool_name, info, {BackgroundColor3 = Color3.fromRGB(27, 158, 88)}):Play()
				end
			end)
			...
			button.MouseLeave:Connect(function()
				...
				tween_service:Create(button.properties.drag_effect, long, {GroupTransparency = 1}):Play()
				tween_service:Create(properties.dragging_element.tool_name, info, {BackgroundColor3 = Color3.fromRGB(65, 122, 255)}):Play()
			end)
...
frame.ChildAdded:Connect(setup_tools)

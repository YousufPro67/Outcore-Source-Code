	...
	for i, items in pairs(frame.player_list:GetChildren()) do
		if items:IsA("ImageButton") then
			...
			items.actions.clear.MouseButton1Click:Connect(function()
				events.banit_events.clear_effects:FireServer(items.properties.username.Value)
			end)

			items.actions.manage.MouseButton1Click:Connect(function()
				local plr = players:GetPlayerByUserId(items.properties.id.Value)

				exe_module:assets_panels("effects", true, plr)
			end)
...
function fetch_effect()
	for i, effect in pairs(exe_storage.effects:GetChildren()) do
		add_effect(effect)
	end
end

function add_effect(effect)
	local template = frame.effects_list.list.effect:Clone()

	template.Name = effect.Name
	template.Parent = frame.effects_list
	template.Size = UDim2.new(0, 0, 0, 60)
	...
	if effect:IsA("Folder") then
		template.to.adornee.Text = "Bulk Effects"
	else
		template.to.adornee.Text = effect:GetAttribute("To")
	end
...
frame.effects_list.list.Changed:Connect(function()
	if frame.effects_list.list.AbsoluteContentSize.Y <= 0 then
...
background.MouseButton1Click:Connect(function()
	exe_module:tools_panels("effects", false)
end)

...
--[=[
	...
	-- Optional if UpdatePreRender should use BindToRenderStep:
	...
	function MyComponent:PreRenderUpdate(dt)
	end
	```
]=]
...
function Component:_setup()
	...
	local function StartComponent(component)
		...
		local hasPreRenderUpdate = typeof(component.PreRenderUpdate) == "function"
		...
		if hasPreRenderUpdate and not IS_SERVER then
			if component.RenderPriority then
				...
				RunService:BindToRenderStep(component._renderName, component.RenderPriority, function(dt)
					component:PreRenderUpdate(dt)
				end)
			else
				component._PreRenderUpdate = RunService.PreRender:Connect(function(dt)
					component:PreRenderUpdate(dt)
				end)
			end
		end
	...
	local function StopComponent(component)
		...
		if component._PreRenderUpdate then
			component._PreRenderUpdate:Disconnect()
...
--[=[
	...
	If this method is present on a component, then it will be
	automatically connected to `RunService.PreRender`. If
	...
	```lua
	-- Example that uses `RunService.PreRender` automatically:

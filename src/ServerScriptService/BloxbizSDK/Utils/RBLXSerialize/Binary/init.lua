	...
	if Data and Data[Type] then 
		...
		if Index == "Value" then 
			...
			if dataSize > 255 then
				Utils.pprint("[SuperBiz][RBXLSerialize][Binary]:Cannot Encode DataValues more than 255 Bytes.")
				return 
			end 
			return (string.char(dataSize)..Type) or 0
		end 
		Utils.pprint("[SuperBiz][RBXLSerialize][Binary]:Could not describe",Index,Type)
		return 0 
	end 
end 
...
function DecodeData(data)
	...
	if StoreType ~= "Invalid" then 
		...
		Utils.pprint("[SuperBiz][RBXLSerialize][Binary]:StoreType defined as Invalid? Binary Data may be corrupted?")
	end
	return parsedTable 
end

...
return function(
	...
	local size = Value(SETTINGS.Size.Default)
	local buttonSize = Spring(size, 40, 1)

	local disconnectSize = Observer(isHovering):onChange(function()
		size:set(isHovering:get() and SETTINGS.Size.Hover or SETTINGS.Size.Default)
	end)

	local background = Value(SETTINGS.ButtonColor.Default)
	local buttonBackground = Spring(background, 40, 1)

	local disconnectBackground = Observer(isHeldDown):onChange(function()
		background:set(isHeldDown:get() and SETTINGS.ButtonColor.MouseDown or SETTINGS.ButtonColor.Default)
	end)
	...
	local function cleanUpCallback()
		...
		disconnectingObserver = nil

		disconnectSize()
		disconnectSize = nil

		disconnectBackground()
		disconnectBackground = nil
	end

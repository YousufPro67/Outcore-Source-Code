...
local cachedGameNames = {}

local IMAGE_FORMAT = "rbxthumb://type=Asset&id=%s&w=150&h=150"

local function getGameName(placeId)
	local name = cachedGameNames[placeId]
	if not name then
		cachedGameNames[placeId] = MarketplaceService:GetProductInfo(placeId).Name
		name = cachedGameNames[placeId]
	end
...
return function(props)
	...
	local date = TimeFormat.GetDate(props.Timestamp)
	local time = TimeFormat.GetTime(props.Timestamp)
	...
	task.spawn(function()
		local image = props.FeedProps.GetUserProfilePicture(profile.UserId)
	...
	local function renderImages()
		if hasScreenshots then
			return buildScreenshot(props.Screenshots[1])
		end

		return isSingleImage and renderSingleImage() or renderMultipleImages()
	end

	props.FeedProps.InteractedWithPostId = props.Id
	props.FeedProps.PostingCommentParent:set(props.Id)
	...
	return New("Frame")({
		...
		[Children] = {
			...
			New("ImageButton")({
				...
				[OnEvent("Activated")] = function()
					props.FeedProps.OnSwitchFeedClicked("replies", props.ParentId)
				end,
			...
			New("Frame")({
				...
				[Children] = {
					...
					New("ImageButton")({
						...
						[OnEvent("Activated")] = function()
							props.FeedProps.OnSwitchFeedClicked(props.FeedProps.initialProfileFeed, profile.UserId)
						end,
					...
					New("Frame")({
						Name = "Profile",
						Size = UDim2.fromScale(0.145, 0.45),
			...
			New("Frame")({
				...
				[Children] = {
					New("Frame")({
						Name = "Info",
						Size = UDim2.fromScale(0.145, 0.4),
						Position = UDim2.fromScale(0.115, 0.3),
						BackgroundTransparency = 1,
						LayoutOrder = 1,
						AutomaticSize = Enum.AutomaticSize.X,
						...
						[Children] = {
							...
							New("TextLabel")({
								...
								Text = getGameName(props.PlaceId),
							}),
						},
					}),
				},

...
function Hologram.New(Prompt: ProximityPrompt, StudsOffset: Vector3?, CheckTag: boolean?, ShowBeam: boolean?)
	...
	self.Colours = {
		Primary = Color3.fromRGB(255, 136, 1),
...
function Hologram:OnPromptShown(InputType: Enum.ProximityPromptInputType)
	...
	local FaceNormal = GetFaceNormals(self.PromptParent)[ClosestFace]
	...
	if self.BillboardActive == true then
		self.BillboardUI = RunService.PreRender:Connect(function()
			if not self.ClonedPrompt or not self.ClonedPrompt.PrimaryPart then return end
			...
			self.DesignPart.CFrame = self.KeyCodePart.CFrame + (self.KeyCodePart.CFrame.LookVector * 1.25)
			self.InstructionPart.CFrame = self.KeyCodePart.CFrame + (self.KeyCodePart.CFrame.LookVector * -1.25) + (self.KeyCodePart.CFrame.RightVector * -1.11)
		end)
	else
		...
		self.DesignPart.CFrame = self.KeyCodePart.CFrame + (self.KeyCodePart.CFrame.LookVector * 1.2)
		self.InstructionPart.CFrame = self.KeyCodePart.CFrame + (self.KeyCodePart.CFrame.LookVector * -1.2) + (self.KeyCodePart.CFrame.RightVector * -1.11)
	end

	if self.ShowBeam == true then
		BorderHighlight.Adornee = self.PromptParent:FindFirstChildOfClass("Model") or self.PromptParent

		local hum:Humanoid = Character:WaitForChild("Humanoid", 10)
		if hum and hum.RigType == Enum.HumanoidRigType.R6 then
			self.Beam.Attachment1 = Character:WaitForChild("Torso").BodyFrontAttachment
		end
	...
	if InputType == Enum.ProximityPromptInputType.Gamepad then
		if GamepadButtonImage[self.Prompt.GamepadKeyCode] then
			self.KeyCodeImage.Image = GamepadButtonImage[self.Prompt.GamepadKeyCode]
		end

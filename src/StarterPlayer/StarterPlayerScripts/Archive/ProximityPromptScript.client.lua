...
local function onLoad()
	local function TweenOnAndOff(obj:Highlight)
		local ts = game:GetService("TweenService")
		local On = ts:Create(obj, TweenInfo.new(0.5), {OutlineTransparency = 0.2})
		local Off = ts:Create(obj, TweenInfo.new(0.5), {OutlineTransparency = 1})
		return On, Off
	end
	ProximityPromptService.PromptShown:Connect(function(prompt, inputType)
		...
		if prompt.Parent then
			if prompt.Parent.Parent:IsA("Model") then
				Subject = prompt.Parent.Parent
			else
				if prompt.Parent:FindFirstChildOfClass("Model") then
					Subject = prompt.Parent:FindFirstChildOfClass("Model")
				else
					Subject = prompt.Parent
				end
			end
		end
		
		local highlight = Subject:FindFirstChild("ProximityHighlight")
		...
		local on,off = TweenOnAndOff(Subject.ProximityHighlight)
		on:Play()
